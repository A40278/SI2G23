...
BEGIN TRAN
	...
	DELETE EquipamentoAlugado WHERE NumeroSerieAluguer = @aluguer AND CódigoEquipamento = @mota;
	--Remove-se um Equipamento sem problemas--
	...
	DELETE EquipamentoAlugado WHERE NumeroSerieAluguer = @aluguer AND CódigoEquipamento = @canoa;
	PRINT('Erro 1, Aluguer já acabou por isso Equipamento não pode ser removido');
	--Erro 1, Aluguer já acabou por isso Equipamento não pode ser removido--
